...
flowchart TD
    PWA[CLIENT PWA] --> SW[Service Worker]
    SW --> Cache[Cache Assets + Offline Page]
    SW --> BgSync[Background Sync + Retry Queue]

    PWA --> Local[Local-First Data Layer]

    Local --> IDB[(IndexedDB Dexie.js<br>Crops • Pests • Listings • Prices)]
    Local --> Queue[Sync Queue<br>• Pending ops<br>• Retry + backoff<br>• Conflict metadata]

    Local <--> Net[Network Detector]

    Net --> Backend[BACKEND  Go + Gin]

    Backend --> API[API Endpoints]
    API --> Sync[POST /sync]
    API --> Prices[GET /prices]
    API --> Pests[GET /pests]

    Backend --> Postgres[(PostgreSQL<br>Users • Crops • Sync Meta)]
    Backend --> Mongo[(MongoDB<br>Listings • Pest Data)]
    Backend --> Redis[(Redis<br>Cache • Sessions)]

    Backend --> Files[Data Sources]
    Files --> CSV[wfp_food_prices_ken.csv]
    Files --> JSON[kenya.json]
  ```
